...
def compute_psi4_SAPT(p4string, method="SAPT0", basis="aug-cc-pVDZ"):
    ...
    _ = psi4.energy(f"{method}", molecule=mol, return_wfn=False)
...
def compute_psi4_BM(p4string, lot="mp2/cc-pv[tq]z + D:ccsd(t)/cc-pvdz"):
    ...
    return psi4.energy(f"{lot}", molecule=mol, return_wfn=False, bsse_type="cp")
...
def run_psi4_calc():
    ...
    df = pd.read_pickle("./co2_nh3_sapt_results_sapt2.pkl")
    ...
    for n, r in df.iterrows():
        ...
        p4string = r["qcel_molecule"].to_string("psi4")
