...
def ap3_d_elst_classical_energies(
    ...
    ap3_path = f"./sft_models/ap3_des370k_{len(finetune_mols)}.pt"
...
def run_finetuning(n_samples, model_type, sft_n_epochs, sft_lr):
    """
    ...
    if n_samples == -1:
        ...
        df_sample = df.sample(
            n=n_samples, random_state=42).reset_index(drop=True)
        finetune_mols = df_sample["qcel_mol"].to_list()
        finetune_labels = df_sample["ref"].to_list()
        n_actual = n_samples
    ...
    print(
        f"Finetuning {model_type.upper()} model with {len(finetune_mols)} samples")
    ...
    if model_type.lower() == "ap2":
        ...
        print(
            f"AP2 model finetuning complete. Model saved to "
        ...
        print(
            f"AP3 model finetuning complete. Model saved to "
            f"./sft_models/ap3_des370k_{len(finetune_mols)}.pt"
        )
    else:
        raise ValueError(
            f"Unknown model type: {model_type}. Must be 'ap2' or 'ap3'")
...
if __name__ == "__main__":
    ...
    run_finetuning(args.N, args.M, sft_n_epochs=args.epochs, sft_lr=args.lr)
