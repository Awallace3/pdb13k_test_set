...
kcalmol_to_kjmol = qcel.constants.cal2J
...
def process_inputs_sapt0_induction(
    ...
    if data_path.endswith(".p4str"):
        ...
        for i in p4input_files:
            ...
            ind20r = subprocess.check_output(
                f"grep '      Ind20,r               ' {i} | awk '{{ print $6 }}'",
                shell=True,
            )
            ind20r = float(ind20r.decode("utf-8"))
            exchind20r = float(subprocess.check_output(
                f"grep '      Exch-Ind20,r               ' {i} | awk '{{ print $6 }}'",
                shell=True,
            ))
            dhf = float(subprocess.check_output(
                f"grep '      delta HF,r (2)           ' {i} | awk '{{ print $6 }}'",
                shell=True,
            ))
            ...
            if verbose:
                ...
                print(geom)
            mol = qcel.models.Molecule.from_data(geom)
            data[Path(i).stem] = [mol, induction, ind20r, exchind20r, dhf, Electrostatics]
    else:
        if verbose:
            print("Unrecognized filetype. Skipping mols")
        data = {}
    ...
    if delta_model_path is None:
        ...
        if len(data) > 0:
            ...
            df_lt['SAPT0 Ind20,r (kJ/mol)'] = [np.nan for i in range(len(df_lt))]
            df_lt['SAPT0 Exch-Ind20,r (kJ/mol)'] = [np.nan for i in range(len(df_lt))]
            df_lt['SAPT0 delta HF,r(2) (kJ/mol)'] = [np.nan for i in range(len(df_lt))]
            for i, (k, v) in enumerate(data.items()):
                dimer = v[0]
                df_lt.loc[df_lt["N-mer Name"] == k, "mol"] = [v[0]]
                df_lt.loc[df_lt["N-mer Name"] == k, "SAPT0 Induction (kJ/mol)"] = [v[1]]
                df_lt.loc[df_lt["N-mer Name"] == k, "SAPT0 Ind20,r (kJ/mol)"] = [v[2]]
                df_lt.loc[df_lt["N-mer Name"] == k, "SAPT0 Exch-Ind20,r (kJ/mol)"] = [v[3]]
                df_lt.loc[df_lt["N-mer Name"] == k, "SAPT0 delta HF,r(2) (kJ/mol)"] = [v[4]]
                df_lt.loc[df_lt["N-mer Name"] == k, "SAPT0 Electrostatics (kJ/mol)"] = [v[5]]
                RA = np.array(dimer.geometry[dimer.fragments[0]], dtype=np.float32) * qcel.constants.bohr2angstroms
                RB = np.array(dimer.geometry[dimer.fragments[1]], dtype=np.float32) * qcel.constants.bohr2angstroms
                ZA = np.array(dimer.atomic_numbers[dimer.fragments[0]], dtype=np.int32)
                ZB = np.array(dimer.atomic_numbers[dimer.fragments[1]], dtype=np.int32)
        ...
        print(df_lt[['N-mer Name', 'SAPT0 Induction (kJ/mol)']])
        return df_lt
...
def dapnet_main_df_crystals(
    ...
    crystal_information = {
        k: {
            ...
            "apprx 1st energy": {k: np.nan for k in delta_model_paths.keys()},
            "apprx+dAPNet2 1st energy": {k: np.nan for k in delta_model_paths.keys()},
        }
        ...
        for k, v in delta_model_paths.items():
            ...
            if not compute_delta:
                df_c_a[f"dapnet cle {v}"] = df_c_a.apply(
                    lambda r: (
                        r[f"Non-Additive MB Energy (kJ/mol) {k}"]
                        + r[f"dAPNet2 apprx {basefile}"] / kcalmol_to_kjmol
                    )
                    * r[f"Num. Rep. (#) {k}"]
                    / int(r[f"N-mer Name apprx"][0]),
                    axis=1,
...
def main():
    ...
    for i in drude_crystals:
        print(i)
        output_data=f"/projects/cos-lab-cs207/ds/awallace43/projects/x23-crystals/{i}/sapt0-dz-aug/{i}/{i}.csv"
        output_data=f"/projects/cos-lab-cs207/ds/awallace43/projects/x23-crystals/{i}/sapt0-dz-aug/{i}/{i}.csv"
        if i == "pyrazine":
            output_data = f"/projects/cos-lab-cs207/ds/awallace43/projects/x23-crystals/{i}/sapt0-dz-aug/{i}/sapt0-dz-aug.csv"
        process_inputs_sapt0_induction(
            output_data=output_data,
            delta_model_path=None,
            data_path=f"/projects/cos-lab-cs207/ds/awallace43/projects/x23-crystals/{i}/sapt0-dz-aug/{i}/*.out",
